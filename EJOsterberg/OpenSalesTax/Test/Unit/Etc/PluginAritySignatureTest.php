...
/**
 * Regression test for **Bug D** (latent v0.1.0 → v1.3.2) and the broader
 ...
 * called with too few args at runtime → ArgumentCountError. This is
 ...
 *     (e.g. `beforeCollect` → `collect` on the target).
 ...
 * mechanism from `DiXmlTargetClassTest` — for each entry, the test
...
{
    /**
     * Target class → method-name → expected arg count. Maintained
    ...
    private const TARGET_METHOD_ARITIES = [
        // vendor/magento/module-tax/Model/Calculation.php — getRate(DataObject $request) → 1 arg
        'Magento\Tax\Model\Calculation' => [
            'getRate' => 1,
        ],
        // vendor/magento/module-tax/Model/Sales/Total/Quote/Tax.php — collect(Quote, ShippingAssignment, Total) → 3 args
    ...
    public function testEveryPluginMethodMatchesTargetArity(): void
    {
            ...
            foreach ($type->plugin as $plugin) {
                ...
                foreach ($reflection->getMethods(ReflectionMethod::IS_PUBLIC) as $method) {
                    ...
                    if ($actualTotal !== $expectedTotal) {
                        $offenders[] = sprintf(
                            '%s::%s — declared %d params, target %s::%s expects %d (so plugin needs %d = %d %s + %d target args). Bug D class.',
                            $pluginClass,
                            $methodName,
                            $actualTotal,
                            $targetClass,
        ...
        self::assertGreaterThan(
            0,
            $checked,
            'No plugin/target arities were checked — TARGET_METHOD_ARITIES is empty or di.xml has no plugins.'
        );
        ...
        self::assertSame(
            ...
            . "with the plugin's declared arg count — too few = ArgumentCountError at runtime, invisible\n"
        ...
        // Don't fail the test for skips — they're a "TODO add coverage" signal,
     ...
     * E.g. `beforeCollect` → ['before', 'collect'].
     *
